...
impl AuthClient {
    pub fn connect(address: &str) -> Result<Self, String> {
        ...
        stream.write_all(client_pk.as_ref()).map_err(|e| e.to_string())?;

        let mut server_pk_bytes = [0u8; KEY_SIZE];
        stream.read_exact(&mut server_pk_bytes).map_err(|e| e.to_string())?;
    ...
    // TOFU (Trust On First Use) 구현 메서드
	#[allow(dead_code)]
...
fn main() {
    ...
    match AuthClient::connect("127.0.0.1:51822") {
